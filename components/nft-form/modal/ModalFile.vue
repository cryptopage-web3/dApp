<template>
  <div class="modal-creat-text mb_20">
    <div class="modal-creat__title2">Image, Video, Audio, Voice</div>
...
export default class ModalFile extends Vue {
  accept = '';
  ...
  validateFileWithNotify(file: File | null): boolean {
    ...
    if (!/(image|video|audio)/.test(fileType)) {
      notify.error('Please upload only image, audio or video');
      return false;
    }

    if (file.size > MAX_FILE_SIZE) {
      notify.error('File size cannot exceed 5 Mb');
      return false;
    }
