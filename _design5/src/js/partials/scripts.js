...
/*start global-request*/
if($('.request-link').is('.request-link')) {
	$('.request-link').on('click', function(){
		event.preventDefault();
		if($(this).attr('data-id')){
			$($(this).attr('data-id')).fadeIn(300);
		}
		$(this).closest('.global-request').fadeOut(300);
	});
}
/*end global-request*/

/*start timer login page*/
if($('.sign-variations__input').is('.sign-variations__input')) {
	function makeTimer(count_seconds) {

		var timeLeft = Number(count_seconds);
		if(timeLeft == -1) {
			...
			$("#sign-variations__timer-wrap").html(minutes + ":" + seconds);
			timeLeft = timeLeft - 1;
			$('.sign-variations__timer').attr('data-time', timeLeft);
		}
	}
	var timerId;
	var start_time = Number($('.sign-variations__timer').attr('data-time'));
	$('.send-code').on('click', function(){
		$('.sign-variations-wrap').slideDown(300);
		timerId = setInterval(function() { makeTimer($('.sign-variations__timer').attr('data-time')); }, 1000);
		$(this).closest('div').fadeOut(300);
	});

	$('.sign-variations__request-code').on('click', function(){
		event.preventDefault();
		$('.sign-variations-wrap').removeClass('active');
		$('.sign-variations__timer').attr('data-time', start_time);
		setTimeout(() => { clearInterval(timerId)}, 1000);
		timerId = setInterval(function() { makeTimer($('.sign-variations__timer').attr('data-time')); }, 1000);
	});
}
/*end timer login page*/
